...
{
    internal const int MaxGenerationRetries = 3;

    /// <summary>Vytvoří novou mapu s náhodně umístěnými loďmi</summary>
    /// <param name="mapSize">Velikost hrací plochy</param>
    internal static SeaMap Generate(int mapSize)
    {
        for (int generationRetry = 0; generationRetry < MaxGenerationRetries; generationRetry++)
        {
            try
            {
                ...
                foreach (Warship warship in warships)
                {
                    ...
                    {
                        throw new MapGenerationException("Nepodařilo se umístit všechny lodě na hrací plochu.");
                    }
        ...
        throw new Exception("Nepodařilo se vygenerovat hrací plochu.");
    }
